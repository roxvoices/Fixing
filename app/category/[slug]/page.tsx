...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://fixmydevice.vercel.app/category/${category.slug}`,
    },
  }
}
...
export default async function CategoryPage({
  ...
  const categoryProblems = ALL_PROBLEMS.filter((p) => p.category === category.id)
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        <div className="pt-32 pb-12 px-4 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {paginatedProblems.length > 0 ? (
              ...
              <div className="col-span-full py-20 text-center bg-slate-100 rounded-3xl border-2 border-dashed border-slate-200">
                ...
                <h3 className="text-lg font-semibold text-slate-900">No guides yet</h3>
          ...
          {totalPages > 1 && (
            <CategoryPagination currentPage={currentPage} totalPages={totalPages} slug={slug} />
          )}
        </div>
      </main>
      <Footer />
