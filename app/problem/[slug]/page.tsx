...
// Tell Next.js to pre-render every single problem slug at build time
// This is what makes every guide indexable by Google
export async function generateStaticParams() {
  return INITIAL_PROBLEMS.map((problem) => ({
    slug: problem.slug,
  }))
}

// Each problem page gets its own unique SEO title + description
export async function generateMetadata({
  ...
  const problem = INITIAL_PROBLEMS.find((p) => p.slug === slug)
  if (!problem) return {}
  ...
  return {
    ...
    alternates: {
      canonical: `https://fixmydevice.com/problem/${problem.slug}`,
    },
  }
}
...
export default async function ProblemPage({
  ...
  const category = CATEGORIES.find((c) => c.id === problem.category)
  const relatedProblems = INITIAL_PROBLEMS.filter((p) =>
    problem.relatedProblemIds.includes(p.id)
  )

  // HowTo structured data — Google can show your steps directly in search results
  const howToSchema = {
  ...
  // BreadcrumbList schema for Google
  const breadcrumbSchema = {
    '@context': 'https://schema.org',
    '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://fixmydevice.com' },
      {
        '@type': 'ListItem',
        position: 2,
        name: category?.name ?? problem.category,
        item: `https://fixmydevice.com/category/${category?.slug ?? problem.category}`,
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: problem.title,
        item: `https://fixmydevice.com/problem/${problem.slug}`,
      },
    ],
  }
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {/* Structured data injected into the page head */}
      <script
      ...
      <main className="flex-1">
        <div className="pt-32 pb-12 px-4 max-w-4xl mx-auto">
          ...
          {/* Symptoms + Causes — rendered as real HTML, fully indexable */}
          ...
          {/* Step-by-Step Fix — rendered as real HTML */}
          <div className="mb-12">
          ...
          {/* Interactive parts (rating, share, comments) — client component */}
